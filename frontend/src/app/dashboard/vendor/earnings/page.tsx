    ...
    return (
        <div className="space-y-8 pb-12">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <StatCard 
                    title="Net Earnings" 
                    value={`$${(metrics?.total_revenue - metrics?.total_commission || 0).toLocaleString()}`} 
                    ...
                    <tbody className="divide-y divide-border">
                        {earnings.map((e) => (
                            <tr key={e.id} className="hover:bg-secondary/5 transition-colors">
                                <td className="px-6 py-4">{new Date(e.created_at).toLocaleDateString()}</td>
                                <td className="px-6 py-4 text-primary">#{e.order_id}</td>
                                <td className="px-6 py-4 text-right">${parseFloat(e.gross_amount).toFixed(2)}</td>
                                <td className="px-6 py-4 text-right text-red-500">-${parseFloat(e.commission_amount).toFixed(2)}</td>
                                <td className="px-6 py-4 text-right font-medium text-green-600">${parseFloat(e.net_amount).toFixed(2)}</td>
