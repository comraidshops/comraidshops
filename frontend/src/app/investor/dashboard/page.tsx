...
import { 
    ...
    Search, Filter, Download, ChevronRight, Globe, Lock
} from 'lucide-react';
...
export default function InvestorDashboard() {
    ...
    return (
        <div className="min-h-screen bg-[#050505] text-white font-sans selection:bg-white selection:text-black">
            ...
            {/* Navigation */}
            <nav className="sticky top-0 z-50 bg-[#050505]/80 backdrop-blur-xl border-b border-white/5 px-8 py-5 flex items-center justify-between">
                ...
                <div className="flex items-center gap-8">
                    ...
                    <button 
                        ...
                        className="flex items-center gap-2 text-[10px] font-bold uppercase tracking-widest text-red-400/60 hover:text-red-400 hover:bg-red-400/5 px-4 py-2 rounded-xl transition-all"
                    >
            ...
            <main className="max-w-7xl mx-auto p-8 lg:p-12 relative z-10 space-y-12">
                {/* Header Section */}
                <header className="flex flex-col md:flex-row md:items-center justify-between gap-8 pb-12 border-b border-white/5">
                    <div className="space-y-4">
                        <div className="flex items-center gap-3">
                            ...
                            <span className="text-white/20 text-[9px] font-bold uppercase tracking-widest">Node: 0xA12...9B2</span>
                        </div>
                        <h1 className="text-5xl font-bold tracking-tighter uppercase leading-none">
                            Welcome, <span className="text-white/40">{profile.user.first_name || profile.user.username}</span>
                        </h1>
                        <p className="text-white/30 text-sm font-medium tracking-wide">Real-time equity analytics & platform governance overview.</p>
                    </div>
                    
                    <div className="flex flex-col items-end gap-2">
                        <div className="bg-white/[0.02] border border-white/10 p-6 rounded-[32px] flex items-center gap-6 shadow-2xl">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                    {[
                        ...
                        <motion.div 
                            ...
                            key={i} 
                            className={`bg-white/[0.02] border border-white/5 p-8 rounded-[40px] shadow-2xl hover:bg-white/[0.04] hover:border-white/10 transition-all group ${metric.glow}`}
                        >
                            <div className={`w-12 h-12 rounded-2xl flex items-center justify-center mb-6 bg-white/5 border border-white/10 group-hover:scale-110 transition-transform`}>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    ...
                    <section className="lg:col-span-2 space-y-8">
                        <div className="flex items-center justify-between px-2">
                            ...
                            <button className="flex items-center gap-2 text-[9px] font-black uppercase tracking-widest text-white/20 hover:text-white transition-colors">
                                <Download className="w-3 h-3" /> Audit Log
                            </button>
                        </div>
                        
                        <div className="bg-white/[0.02] border border-white/5 rounded-[48px] p-10 shadow-2xl space-y-12">
                                ...
                                {allocations.map((alloc: any, i: number) => (
                                    <div key={i} className="space-y-4">
                                        <div className="space-y-1">
                                            ...
                                            <span className="text-2xl font-bold text-white">{alloc.allocation_percentage}%</span>
                                        </div>
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    {allocations.map((alloc: any, i: number) => (
                                        <div key={i} className="bg-white/[0.02] border border-white/5 p-6 rounded-3xl hover:bg-white/[0.04] transition-all">
                                            <span className="text-[9px] font-black uppercase tracking-widest text-white/40 block mb-3 underline decoration-white/10 underline-offset-4">{alloc.category_display}</span>
                                            <p className="text-xs text-white/50 leading-relaxed font-medium">{alloc.description_logs}</p>
                                        </div>
                                    ))}
                                </div>
                            </div>
                    ...
                    <section className="space-y-8">
                        <h2 className="text-xl font-bold tracking-tight uppercase flex items-center gap-3 px-2">
                            <Milestone className="w-6 h-6 text-white/20" />
                            Roadmap Node
                        </h2>
                        <div className="bg-white/[0.02] border border-white/5 rounded-[48px] p-10 shadow-2xl relative overflow-hidden">
                            <div className="absolute top-10 bottom-10 left-[55px] w-px bg-white/5"></div>
                            <div className="space-y-12 relative">
                                {milestones.map((milestone: any, i: number) => (
                                    <div key={i} className="flex gap-8 group">
                                        <div className={`w-8 h-8 rounded-xl flex-shrink-0 flex items-center justify-center z-10 border border-white/10 ${
                                        ...
                                        <div className="space-y-2">
                                            <div className="flex items-center gap-3">
                                                <span className="text-[10px] font-black uppercase tracking-tighter text-white/20">{milestone.date}</span>
                                            ...
                                            <h3 className="text-sm font-bold text-white group-hover:translate-x-1 transition-transform">{milestone.title}</h3>
                                            <p className="text-[11px] text-white/30 leading-relaxed font-medium">{milestone.description}</p>
                                        </div>
                                    </div>
                                ))}
                            </div>
                ...
                <section className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                    {/* Valuation Card */}
                    <div className="bg-white text-black rounded-[48px] p-10 flex flex-col justify-between overflow-hidden relative group shadow-2xl">
                        ...
                        <div className="relative z-10 space-y-8">
                            <div className="space-y-2">
                                ...
                                <h2 className="text-6xl font-bold tracking-tighter leading-none">${parseFloat(company_valuation || 0).toLocaleString()}</h2>
                            </div>
                            <div className="flex flex-wrap gap-4">
                                <div className="flex items-center gap-3 bg-black/5 px-6 py-3 rounded-2xl border border-black/5">
                                    <TrendingUp className="w-5 h-5 text-black" />
                                    <span className="text-[10px] font-bold uppercase tracking-widest">Growth Node 4.0</span>
                                </div>
                                <div className="flex items-center gap-3 bg-black/5 px-6 py-3 rounded-2xl border border-black/5">
                                    <Lock className="w-5 h-5 text-black" />
                                    <span className="text-[10px] font-bold uppercase tracking-widest">Verified Q1</span>
                                </div>
                            </div>
                        </div>
                        <div className="mt-16 flex items-start gap-4 p-6 bg-black/5 rounded-3xl border border-black/5 backdrop-blur-sm">
                            <Info className="w-5 h-5 text-black/40 flex-shrink-0 mt-1" />
                            <div className="space-y-1">
                                <span className="text-[10px] font-black uppercase tracking-[0.2em] block mb-2">Legal Disclaimer</span>
                                <p className="text-[10px] text-black/50 leading-relaxed font-medium uppercase tracking-tighter">
                    ...
                    <div className="space-y-8">
                        ...
                        <div className="space-y-4">
                            {latest_updates.map((update: any, i: number) => (
                                <div key={i} className="bg-white/[0.02] border border-white/5 p-8 rounded-[40px] shadow-2xl hover:bg-white/[0.04] hover:border-white/10 transition-all group">
                                    <div className="flex items-center justify-between mb-4">
                                        <span className="text-[9px] font-black uppercase tracking-[0.3em] text-white/20">{new Date(update.created_at).toLocaleDateString()}</span>
                                        <span className="text-[8px] font-bold uppercase tracking-widest bg-white/5 text-white/40 px-4 py-2 rounded-xl border border-white/5 group-hover:bg-white group-hover:text-black transition-all">
                                            {update.category}
                                        </span>
                                    </div>
                                    <h3 className="text-lg font-bold mb-3 group-hover:translate-x-2 transition-transform text-white/90">{update.title}</h3>
                                    <p className="text-xs text-white/30 leading-relaxed font-medium line-clamp-2">{update.content}</p>
                                </div>
                            ))}
                        </div>
                    </div>
                ...
                {/* Footer Action */}
                <section className="bg-white/[0.02] border border-white/5 rounded-[64px] p-16 text-center space-y-10 shadow-2xl relative overflow-hidden group">
                    ...
                    <div className="max-w-2xl mx-auto space-y-6 relative z-10">
                        <div className="bg-white/5 w-20 h-20 rounded-[32px] flex items-center justify-center mx-auto mb-10 border border-white/10 group-hover:rotate-12 transition-transform">
                            <Globe className="w-10 h-10 text-white/40" />
                        </div>
                        <h2 className="text-4xl font-bold tracking-tighter uppercase leading-none">Strategic Expansion Q4</h2>
                        <p className="text-white/30 text-lg font-medium leading-relaxed tracking-wide">Indicate interest for upcoming Series A funding rounds or request a direct strategic session with the founding team.</p>
                    </div>
                    
                    <div className="flex flex-col sm:flex-row items-center justify-center gap-6 relative z-10">
                        <button className="w-full sm:w-auto bg-white text-black px-12 py-5 rounded-2xl text-[10px] font-bold uppercase tracking-[0.2em] flex items-center justify-center gap-3 hover:scale-105 transition-all shadow-[0_0_40px_rgba(255,255,255,0.15)]">
                            Direct Contact <ExternalLink className="w-4 h-4" />
                        </button>
                        <button className="w-full sm:w-auto bg-transparent border border-white/10 text-white/40 px-12 py-5 rounded-2xl text-[10px] font-bold uppercase tracking-[0.2em] hover:bg-white/5 hover:text-white transition-all">
            ...
            {/* Global Footer */}
            <footer className="border-t border-white/5 py-16 px-12 bg-transparent relative z-10">
                <div className="max-w-7xl mx-auto flex flex-col md:flex-row items-center justify-between gap-12">
                    <div className="flex flex-col items-center md:items-start gap-4">
                        <div className="flex items-center gap-3">
                            ...
                            <span className="text-[10px] font-black uppercase tracking-[0.4em] text-white/20">© 2026 COMRAIDSHOPS INC.</span>
                        </div>
                    </div>
                    <div className="flex flex-wrap justify-center gap-10 text-[9px] font-bold uppercase tracking-widest text-white/20">
