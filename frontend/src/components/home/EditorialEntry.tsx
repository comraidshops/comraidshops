...
export default function EditorialEntry({ initialArticle }: EditorialEntryProps) {
    ...
    const getLongPreview = () => {
        const rawContent = (magazine.articles?.[0]?.content || magazine.description || "").trim();
        ...
        // Diagnostic: Only use the narrative if the content matches the exact placeholder or is completely blank
        const isPlaceholder = stripped.toLowerCase().includes("discover the latest perspective") || stripped === "";
