...
class MagazineViewSet(viewsets.ReadOnlyModelViewSet):
    queryset = Magazine.objects.select_related('article').all()
    def get_queryset(self):
        qs = Magazine.objects.select_related('article').all()
...
class MagazineFeaturedView(APIView):
    ...
    def get(self, request):
        magazine = Magazine.objects.filter(is_featured=True).select_related('article').order_by('-created_at').first()
        if not magazine:
